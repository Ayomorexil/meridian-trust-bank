...
async function seed() {
  ...
  await db.tx(async (t) => {
    ...
    const holdings = [
      ["TSLA", "Tesla, Inc.", 220, 189.4, 248.12],
      ["AAPL", "Apple Inc.", 400, 165.2, 214.88],
      ["NVDA", "NVIDIA Corporation", 150, 410.0, 612.55],
      ["VOO", "Vanguard S&P 500 ETF", 500, 380.0, 452.3],
      ["MSFT", "Microsoft Corporation", 180, 310.0, 398.21],
    ];
    ...
    const charges = [
      [
        checking.id,
        "Internal Revenue Service — Est. Tax",
        "taxes",
        4200.0,
        "quarterly",
        nextMonth(15),
      ...
      [
        checking.id,
        "Meridian Trust Mortgage Payment",
        "bills",
        3120.0,
        "monthly",
        nextMonth(1),
    ...
    console.log(
      "Seeded successfully. This is entirely simulated demo data — no real funds, no real institution.\n",
    );
    console.log(
      "  Superadmin login: superadmin@meridiantrust.demo / SuperAdmin!2345",
    );
    console.log("  Admin login:      admin@meridiantrust.demo / Admin!2345");
    console.log(
      "  Member login:     edward.etkinson@example.com / Password!2345",
    );
    console.log("  Member login:     priya.n@example.com / Password!2345");
    console.log(
      "  Member login (KYC pending — try approving in the admin console): james.okafor@example.com / Password!2345",
    );
    console.log(
